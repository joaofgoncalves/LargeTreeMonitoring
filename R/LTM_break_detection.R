...
# Randomized, recovery-aware trend validator around a break ----------------
# Uses a single centered window around `brk` and compares its Theil-Sen slope
# to a null distribution of slopes from random, equally-long, contiguous
# pre-break windows. Works in %/year, relative to a robust pre baseline.
...
ltm_ed_detect_breaks <- function(spidf,
                                 ...
                                 st_fun         = median,   # mean/median for short-term windows
...
if (break_select == "largest_drop") {
  changes <- sapply(brk_candidates, function(bi) {
    if (is.null(tresh_int)) {
      pre_val  <- suppressWarnings(thresh_fun(ysa[1:(bi - 1)], na.rm = TRUE))
      post_val <- suppressWarnings(thresh_fun(ysa[(bi + 1):n], na.rm = TRUE))
    } else {
      pre_rng  <- bound_idx(bi - tresh_int, bi - 1, n)
      post_rng <- bound_idx(bi + 1, bi + tresh_int, n)
      pre_val  <- suppressWarnings(thresh_fun(ysa[pre_rng],  na.rm = TRUE))
      post_val <- suppressWarnings(thresh_fun(ysa[post_rng], na.rm = TRUE))
    }
...
brk_candidates[1]
  ...
  if (is.null(tresh_int)) {
    pre_break  <- suppressWarnings(thresh_fun(ysa[1:(brk - 1)], na.rm = TRUE))
    post_break <- suppressWarnings(thresh_fun(ysa[(brk + 1):n], na.rm = TRUE))
  } else {
    pre_rng  <- bound_idx(brk - tresh_int, brk - 1, n)
    post_rng <- bound_idx(brk + 1, brk + tresh_int, n)
    pre_break  <- suppressWarnings(thresh_fun(ysa[pre_rng],  na.rm = TRUE))
    post_break <- suppressWarnings(thresh_fun(ysa[post_rng], na.rm = TRUE))
  }
...
ltm_bfast01_detect_breaks <- function(
  ...
  val_pre  <- suppressWarnings(thresh_fun(pre,  na.rm = TRUE))
...
ltm_mcp_detect_breaks <- function(spidf,
  ...
  # Long-term magnitude using intercepts (kept) ---------------------------
  pre_break  <- pars[2, 2]
  post_break <- pars[3, 2]
  break_magn <- pct_change(post_break, pre_break)
...
ltm_strucchange_detect_breaks <- function(
  ...
  val_pre    <- suppressWarnings(thresh_fun(pre_vals,  na.rm = TRUE))
