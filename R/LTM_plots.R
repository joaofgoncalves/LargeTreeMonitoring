...
#'
#' Builds a `ggplot` visualization of available spectral-index series stored in
#' an `spidf` object. The plot can overlay break dates from a flat
#' break-detection data frame and optionally restrict them to validator-approved
#' breaks.
#'
#' @param spidf_obj An object of class `spidf`.
#' @param tree_id Optional tree identifier shown in the plot subtitle when not
#'   empty and not `"---"`.
#' @param df_breaks Optional data frame of break-detection results, typically
#'   produced by [as.data.frame.ts_breaks()].
#' @param only_valid_breaks Logical scalar. If `TRUE`, plot only breaks that
#'   pass at least one selected validator or all selected validators according
#'   to `valid_breaks_mode`.
#' @param valid_breaks_mode Character scalar. Either `"any"` to retain breaks
#'   passing any selected validator or `"all"` to retain breaks passing all
#'   selected validators.
#' @param validators_sel Optional character vector selecting validator groups
#'   used when `only_valid_breaks = TRUE`. Supported values are `"lt"`,
#'   `"st"`, and `"st_trend"`. When `NULL`, all available validator columns are
#'   considered.
#'
...
ltm_plot_spidf_ts <- function(spidf_obj, tree_id = "",
  ...
  stopifnot(is.logical(only_valid_breaks), length(only_valid_breaks) == 1L,
            !is.na(only_valid_breaks))
  ...
  series_cols <- c(
    "spi"            = ifelse(is_regularized(spidf_obj),
                              "Daily interpolated",
                              "Original SPI"),
  ...
  if (!is.null(df_breaks)) {
    ...
    if (only_valid_breaks) {
      if (length(validator_cols_present) == 0L) {
        warning(
          "`only_valid_breaks = TRUE`, but no validator columns
          were found in `df_breaks`. ",
  ...
  base_line_types <- c("solid", "dashed",
                       "dotted", "dotdash",
                       "longdash", "twodash")
...
#' detections. The first panel shows the frequency of valid break dates and the
#' second panel shows the distribution of break magnitudes by algorithm.
#'
#' @param df_breaks Data frame containing at least `algorithm`, `break_date`,
#'   `break_magn`, and `has_valid_breaks_lt` columns.
#'
#' @return A patchwork object combining two `ggplot` plots.
#' @family plotting helpers
#' @export
plot_valid_breaks <- function(df_breaks) {

  required_cols <- c("algorithm",
                     "break_date",
                     "break_magn",
                     "has_valid_breaks_lt")
  ...
  df_valid <- df_breaks %>%
    dplyr::filter(has_valid_breaks_lt == TRUE, !is.na(break_date),
                  !is.na(break_magn))
  ...
  p1 <- ggplot(freq_data,
               aes(x = reorder(as.character(break_date), freq), y = freq)) +
